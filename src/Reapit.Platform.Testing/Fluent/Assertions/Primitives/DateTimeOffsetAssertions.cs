...
/// <summary>Contains assertions for DateTimeOffset types.</summary>
/// <param name="subject">The assertion subject.</param>
[DebuggerNonUserCode]
public class DateTimeOffsetAssertions(DateTimeOffset subject)
    : NullableDateTimeOffsetAssertions<DateTimeOffsetAssertions>(subject);
    ...
    where TAssertions : NullableDateTimeOffsetAssertions<TAssertions>
{
    ...
    protected const string Context = "datetimeoffset";

    /// <summary>The subject of assertions.</summary>
    protected DateTimeOffset? Subject { get; } = subject;
    ...
    public AndOperator<TAssertions> NotBe(DateTimeOffset compareTo)
    {
        if (Subject?.CompareTo(compareTo) != 0)
            return new AndOperator<TAssertions>((TAssertions)this);
    ...
    public AndOperator<TAssertions> BeCloseTo(DateTimeOffset compareTo, TimeSpan precision)
    {
    ...
    /// <exception cref="ArgumentException">when <paramref name="precision"/> is negative.</exception>
    public AndOperator<TAssertions> NotBeCloseTo(DateTimeOffset compareTo, TimeSpan precision)
    {
        if (precision < TimeSpan.Zero)
            throw new ArgumentException("Precision cannot be a negative value.", nameof(precision));
    ...
    public AndOperator<TAssertions> BeAfter(DateTimeOffset compareTo)
    {
        if (Subject?.CompareTo(compareTo) > 0)
            return new AndOperator<TAssertions>((TAssertions)this);
